...
<body>
    <div class="wrapper">
        <div class="header"><h1>{{ __('Reset Password') }}</h1></div>
        <div class="body">
            <p>{{ __('Hello!') }}</p>
            ...
            <p class="text-muted">{{ __('This password reset link will expire in :count minutes.', ['count' => config('auth.passwords.users.expire', 60)]) }}</p>
            <p class="text-muted">{{ __('If you did not request a password reset, no further action is required.') }}</p>
        </div>
        <div class="footer">
