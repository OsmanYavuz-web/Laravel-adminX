...
class Edit extends Component
{
    ...
    {
        $f = $this->find;
        $this->findDate             = $f->find_date ? \Illuminate\Support\Carbon::parse($f->find_date)->format('Y-m-d') : '';
    ...
    public function save(): void
    {
        ...
        if (is_array($this->newGallery)) {
            foreach ($this->newGallery as $file) {
                if ($file) $this->find->addMedia($file)->toMediaCollection('gallery');
            }
        }
        if (is_array($this->newDocuments)) {
            foreach ($this->newDocuments as $file) {
                if ($file) $this->find->addMedia($file)->toMediaCollection('document');
            }
        }
